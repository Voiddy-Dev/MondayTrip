...
interface UseRemoveTripParticipantProps {
  networkId: number;
  tripId: number;
  participant: string;
}

const useRemoveTripParticipant = ({
  networkId,
  tripId,
  participant
}: UseRemoveTripParticipantProps): UseWriteTransactionResponse => {
  const {
    config,
    error: prepareError,
    ...
    abi: plannerAbi,
    functionName: 'removeTripParticipant',
    chainId: networkId,
    args: [
      BigInt(tripId),
...
export default useRemoveTripParticipant;